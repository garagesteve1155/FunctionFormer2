import os, re, threading, time, subprocess, sys, json, tempfile, tkinter as tk, requests
...
# === ChatGPT (requests) config ===
# Using OpenAI's Chat Completions over raw HTTP via `requests`.
# We keep names like SNAPSHOT_ROOT and _slugify_for_fs intact for the rest of the app.
OPENAI_KEY_FILE = os.path.join(os.path.dirname(__file__), "openai_api_key.txt")
OPENAI_API_KEY: str | None = None

# Will be chosen by the user in a popup before the goal prompt.
CHATGPT_MODEL: str | None = None
CHATGPT_DEFAULT_MODEL = "gpt-4o-mini"

def load_openai_api_key() -> str | None:
    """Load a previously saved API key from disk, if present."""
...
def save_openai_api_key(key: str) -> None:
    """Persist API key and cache in memory for this process."""
    global OPENAI_API_KEY
    OPENAI_API_KEY = (key or "").strip()
    try:
        if OPENAI_API_KEY:
...
# Preload any saved key (may be None)
load_openai_api_key()
...
def discover_frameworks() -> list[dict]:
    ...
    for name in os.listdir(FRAMEWORKS_DIR):
        if not any(name.lower().endswith(ext) for ext in (".json", ".txt", ".md", ".pyfw")):
            continue
...
def llm(prompt: str, **gen_kwargs) -> str:
    ...
    messages = build_messages(trimmed_pairs)
    while total_chars(messages) > 16000 and trimmed_pairs:
        trimmed_pairs.pop(0)
        messages = build_messages(trimmed_pairs)
    ...
    model_name = (CHATGPT_MODEL or CHATGPT_DEFAULT_MODEL)
    api_key = OPENAI_API_KEY
    if not model_name:
        return "ERROR: No ChatGPT model selected."
    if not api_key:
...
class ChatGUI:
    ...
    def show_goal_popup(self):
        """
        New sequence:
        1) Popup to select which ChatGPT model to use.
        2) If no saved API key, popup to enter it and save.
        3) Then show the original Goal popup.
        """
        def _ask_goal():
        ...
        def _after_key_entry(key_text: str):
            key_text = (key_text or "").strip()
            if key_text:
                save_openai_api_key(key_text)
            _ask_goal()

        def _ensure_key_then_goal():
            if not (OPENAI_API_KEY or load_openai_api_key()):
                self.open_input_popup(
                    "Enter your OpenAI API key (starts with 'sk-'):\nIt will be saved locally for future runs.",
                    "Save Key (Ctrl+Enter)",
                    _after_key_entry
                )
            else:
        ...
        def _set_model_and_continue(selected: str):
            global CHATGPT_MODEL
            CHATGPT_MODEL = (selected or "").strip() or CHATGPT_DEFAULT_MODEL
            _ensure_key_then_goal()

        # --- Model selection popup (simple radiobuttons) ---
        win = tk.Toplevel(self.root)
        win.title("Select ChatGPT Model")
        try:
            win.configure(bg=self.BG)
        except Exception:
            pass
        ...
        tk.Label(
            win,
            text="Select which ChatGPT model to use:",
            justify="left",
            bg=getattr(self, "BG", None) or None,
            fg=getattr(self, "FG", None) or None,
            font=("Consolas", 10, "bold")
        ).pack(padx=12, pady=(12, 8), anchor="w")

        model_var = tk.StringVar(value=CHATGPT_MODEL or CHATGPT_DEFAULT_MODEL)
        options = ["gpt-5", "gpt-5-mini", "gpt-5-nano", "gpt-4.1", "gpt-4.1-mini", "gpt-4.1-nano", "gpt-4o", "gpt-4o-mini"]
        for m in options:
            tk.Radiobutton(
                win, text=m, variable=model_var, value=m,
                bg=getattr(self, "BG", None) or None,
                fg=getattr(self, "FG", None) or None,
                selectcolor=getattr(self, "BG_PANEL", None) or None,
                activebackground=getattr(self, "ACCENT_BG", None) or None,
                activeforeground=getattr(self, "FG", None) or None,
        ...
        def _confirm():
            try:
                ...
                pass
            win.destroy()
            _set_model_and_continue(model_var.get())

        btn = tk.Button(win, text="Use This Model", command=_confirm)
        try:
            btn.configure(
                bg=self.ACCENT_BG, fg=self.FG, activebackground=self.ACCENT,
                activeforeground=self.BG, relief="flat", bd=0, padx=12, pady=6,
            ...
            pass
        btn.pack(padx=12, pady=(10, 12), anchor="e")
